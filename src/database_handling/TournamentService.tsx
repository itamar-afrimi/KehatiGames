...
export const addTournament = async (title: string, participants: string[]): Promise<void> => {
    ...
    const simpleDoc = { test: 'Simple document' };
    const simpleDocRef = await firestore().collection('tournaments').add(simpleDoc);
    console.log('Simple document added:', simpleDocRef.id);
    try{
        const tournament = {
            title,
            creator: userID,
